...
export default function FactoriesPage() {
  ...
  const openEdit = (f: Factory) => {
    setEditTarget(f);
    setForm({ name: f.name, code: f.code, address: f.address || "", phone: f.phone || "" });
    setError("");
    setDialogOpen(true);
  };
          ...
          <Table>
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={9} align="center" sx={{ py: 6, color: "text.secondary" }}>등록된 매장이 없습니다</TableCell></TableRow>
              ) : factories.map((f) => (
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} maxWidth="sm" fullWidth>
        ...
        <DialogContent>
          ...
          <Stack spacing={2} sx={{ mt: 1 }}>
            ...
            <TextField label="코드 *" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value.toUpperCase() })} fullWidth helperText="영문 대문자, 숫자 조합" disabled={!!editTarget} />
